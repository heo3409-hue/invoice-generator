...
# 거래처별 shared strings 인덱스 (F3=InvoiceNo, F4=InvoiceDate)
SS_INDEX = {
...
def update_shared_string(ss_xml, index, new_value):
    """sharedStrings.xml에서 특정 인덱스의 텍스트 교체"""
    ...
    with zipfile.ZipFile(template_path, 'r') as zin:
        with zipfile.ZipFile(out, 'w', zipfile.ZIP_DEFLATED) as zout:
            for item in zin.namelist():
                ...
                # sharedStrings.xml: Invoice No, Date 교체
...
@app.route('/health')
def health():
    return jsonify({'status': 'ok', 'version': 'v4-sharedstrings'})
