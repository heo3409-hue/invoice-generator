...
import io, os, zipfile, re, subprocess, tempfile
...
def set_inline_str_cell(content, cell_ref, value):
    ...
    if new == content:
        new = re.sub(rf'(<c r="{cell_ref}"[^>]*><is><t>)[^<]*(</t></is></c>)', rf'\g<1>{escaped}\g<2>', content)
    return new
...
def xlsx_to_pdf(xlsx_buf, inv_no):
    with tempfile.TemporaryDirectory() as tmpdir:
        xlsx_path = os.path.join(tmpdir, f'{inv_no}.xlsx')
        ...
        pdf_path = os.path.join(tmpdir, f'{inv_no}.pdf')
...
def generate_pdf():
    ...
    inv_no = data.get('inv_no', 'TS-000000-01')
    updates_num, updates_str = get_updates(data)
    xlsx_buf = generate_xlsx(TEMPLATES[cid], updates_num, updates_str)
    pdf_buf = xlsx_to_pdf(xlsx_buf, inv_no)
    if not pdf_buf:
